...
const {pipelinesFilePath, sitesProps} = require('../config/parse');
const _PRIVATE_TOKEN = '';
...
module.exports = async function parseDataFromGit() {
	...
	for (const site of sitesProps) {
		let url = `https://git.fabricmedia.ru/api/v4/projects/${site.repoId}/pipelines?ref=${_BRANCH_NAME}`;
	...
	for (const site of pipelines) {
		...
		for (const id of site.pipelinesIds) {
			pipelinesFetchPull.push(
				fetch(`https://git.fabricmedia.ru/api/v4/projects/${repoId}/pipelines/${id}`, {
					method: 'GET',
					headers: {
						'private-token': _PRIVATE_TOKEN,
						'content-type': 'application/json'
